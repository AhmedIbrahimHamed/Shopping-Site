  ...
  autoLogin() {
    ...
    if (!userData) {
      ...
      const loadedUser = new User(
        ...
        userData.id,
        userData._token,
        new Date(userData._tokenExprationData)
      );

      if (loadedUser.token) {
        this.user.next(loadedUser);
        const expirationDuration = new Date(userData._tokenExprationData).getTime() - new Date().getTime();
